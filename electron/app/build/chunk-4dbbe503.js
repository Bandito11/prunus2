...
function dateToString(date, opts) {
    let currentDate;
    if (opts === 'stats') {
        if (date.getMonth() < 10) {
...
var Capacitor$1 = new CapacitorWeb();
Capacitor$1 = window.Capacitor || Capacitor$1;
// Export window.Capacitor if not available already (ex: web)
if (!window.Capacitor) {
    window.Capacitor = Capacitor$1;
}
...
class CapacitorDataStorageSqlitePluginWeb extends WebPlugin {
    constructor() {
        super({
 ...
 */
function CapacitorStorageAdapter() { }
/**
...
const lokiOptions = {
    ...
    autoloadCallback: () => {
        if (!logsColl) {
            logsColl = prunusDB.addCollection('logs');
        }
        else {
            logsColl = prunusDB.getCollection('logs');
        }
    }
};
let prunusDB = new loki('prunus.db', lokiOptions);
...
function getLogs(date) {
    ...
    try {
        ...
        if (record) {
            response = Object.assign({}, response, { success: true, data: record.length });
            return response;
        }
        else {
            response = Object.assign({}, response, { error: 'There were no records on DB for this date.' });
            return response;
        }
    }
    catch (error) {
        ...
        response = Object.assign({}, response, { success: true, data: -1 });
        return response;
    }
}
function insert(log) {
    ...
    if (results) {
        response = Object.assign({}, response, { error: 'Entry is already on DB in the database.' });
    }
    else {
...
function update(log) {
    ...
    try {
        ...
        response = Object.assign({}, response, { error: error });
    }
    return response;
}
function remove(log) {
    ...
    if (results) {
        ...
        response = Object.assign({}, response, { success: true, error: null, data: logsColl.addDynamicView('logs') });
    }
    else {
        response = Object.assign({}, response, { error: 'This record doesn\'t exist on database.', data: logsColl.addDynamicView('logs') });
    }
    return response;
}
function clear() {
    ...
    try {
        logsColl.clear();
        response = Object.assign({}, response, { success: true, error: null });
    }
    catch (error) {
        response = Object.assign({}, response, { error: error });
    }
...
export { getLogsView as a, update as b, formatTime as c, dateToString as d, insert as e, remove as f, clear as g, toISOFormat as h, getLogs as i };
